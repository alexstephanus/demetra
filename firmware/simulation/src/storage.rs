...
pub fn create_config_storage() -> Result<SimpleRingBuffer<DeviceConfig, MockFlashStorage, MockFlashStorageError>> {
    ...
    Ok(SimpleRingBuffer::new(start_address, end_address, mock_flash).expect("simulation storage addresses must be page-aligned"))
}