...
mod tests {
    ...
    fn make_calibration(cal_resistance: f32, solution_us: f32, cal_temp_c: f32) -> ConductivityCalibration {
    ...
    fn measurement_at_calibration_resistance_and_temperature_returns_solution_value() {
        ...
        let result = cal.get_conductivity(Resistance::from_ohms(1000.0), Temperature::from_celsius(25.0));
        let diff = (result.us_per_cm() - 1413.0).abs();
        assert!(diff < 0.01, "Expected ~1413 µS/cm, got {:.4}", result.us_per_cm());
    }

    #[test]
    fn higher_resistance_gives_lower_conductivity() {
        let cal = make_calibration(1000.0, 1413.0, 25.0);
        let low_r = cal.get_conductivity(Resistance::from_ohms(800.0), Temperature::from_celsius(25.0));
        let high_r = cal.get_conductivity(Resistance::from_ohms(1200.0), Temperature::from_celsius(25.0));
    ...
    fn temperature_compensation_higher_temp_reports_lower_ec() {
        let cal = make_calibration(1000.0, 1413.0, 25.0);
        let at_25 = cal.get_conductivity(Resistance::from_ohms(1000.0), Temperature::from_celsius(25.0));
        let at_35 = cal.get_conductivity(Resistance::from_ohms(1000.0), Temperature::from_celsius(35.0));
        assert!(
            at_35.us_per_cm() < at_25.us_per_cm(),
            "Higher temperature should yield lower compensated EC. 25°C: {:.2}, 35°C: {:.2}",
            at_25.us_per_cm(), at_35.us_per_cm()
        );
    }
    ...
    fn temperature_compensation_matches_formula() {
        let cal = make_calibration(1000.0, 1413.0, 25.0);
        let t_meas = 20.0_f32;
        let result = cal.get_conductivity(Resistance::from_ohms(1000.0), Temperature::from_celsius(t_meas));
        let expected = 1413.0 * 1.0 / (1.0 + EC_TEMP_COEFF * (t_meas - EC_REF_TEMP_C));
        let diff = (result.us_per_cm() - expected).abs();
        assert!(diff < 0.01, "Expected {:.4}, got {:.4}", expected, result.us_per_cm());
    }
    ...
    fn calibration_at_non_reference_temperature_compensates_correctly() {
        ...
        let result = cal.get_conductivity(Resistance::from_ohms(cal_resistance), Temperature::from_celsius(meas_temp));
        let expected = solution_us_25
            * (1.0 + EC_TEMP_COEFF * (cal_temp - EC_REF_TEMP_C))
    ...
    fn cell_constant_scales_correctly() {
        ...
        assert!(diff < 0.01, "Expected K≈1.0, got {:.4}", cal_k_1.cell_constant);
        ...
        assert!(diff < 0.01, "Expected K≈2.0, got {:.4}", cal_k_2.cell_constant);
        ...
        assert!(diff < 0.01, "Expected K≈0.5, got {:.4}", cal_k_0_5.cell_constant);
    }
