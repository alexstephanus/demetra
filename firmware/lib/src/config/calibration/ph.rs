...
impl ThreePointPhCalibration {
    ...
    fn calculate_slope_and_offset(
        ...
        if mid.ph_value <= NEUTRAL_PH {
            let slope_high = ideal_temperature.kelvin()
                * (high.measured_voltage.mv() - mid.measured_voltage.mv())
            ...
            let calibration_offset = Voltage::from_mv(
                mid.measured_voltage.mv()
                    + (slope_high * mid.temperature.kelvin() / ideal_temperature.kelvin()
                        * (NEUTRAL_PH - mid.ph_value)),
            );
            let slope_low = ideal_temperature.kelvin()
                * (calibration_offset.mv() - low.measured_voltage.mv())
                / (low.temperature.kelvin() * (NEUTRAL_PH - low.ph_value));
            (slope_high, slope_low, calibration_offset)
        } else {
            let slope_low = ideal_temperature.kelvin()
                * (mid.measured_voltage.mv() - low.measured_voltage.mv())
                / ((mid.ph_value * mid.temperature.kelvin())
            ...
            let slope_high = (high.measured_voltage.mv() - calibration_offset.mv())
                * ideal_temperature.kelvin()
    ...
    pub fn get_calibrated_ph_measurement(
        ...
        let ph_value: PhValue = NEUTRAL_PH
            + (measured_voltage.mv() - self.mv_offset.mv()) / temperature_compensated_slope;

        PhMeasurementPoint::new(ph_value, measured_voltage, temperature)
    }
...
mod test_ph_calibration {
    ...
    fn test_ph_calibration_standard() {
        let low_ph = PhMeasurementPoint::new(
            4.00,
            Voltage::from_mv(57.0 * 3.0),
            Temperature::from_celsius(25.0),
        );
        let mid_ph =
            PhMeasurementPoint::new(7.00, Voltage::from_mv(0.0), Temperature::from_celsius(25.0));
        let high_ph = PhMeasurementPoint::new(
            10.00,
            Voltage::from_mv(-57.0 * 3.0),
            Temperature::from_celsius(25.0),
        );
    ...
    fn test_ph_calibration_6_mid() {
        ...
        let mid_ph = PhMeasurementPoint::new(
            6.00,
            Voltage::from_mv(57.0),
            Temperature::from_celsius(25.0),
        );
        let high_ph = PhMeasurementPoint::new(
            10.00,
    ...
    fn test_measurement_at_calibration_points() {
        ...
        assert_float_equality(
            cal.get_calibrated_ph_measurement(t, Voltage::from_mv(57.0 * 3.0))
                .ph_value,
            4.0,
        );
        assert_float_equality(
            cal.get_calibrated_ph_measurement(t, Voltage::from_mv(0.0))
                .ph_value,
            7.0,
        );
        assert_float_equality(
    ...
    fn test_measurement_temperature_compensation() {
        ...
        let expected_low =
            7.0 + low_ph_mv / (cal.slope_low * t_measured.kelvin() / t_ideal.kelvin());
        assert_float_equality(
            cal.get_calibrated_ph_measurement(t_measured, Voltage::from_mv(low_ph_mv))
                .ph_value,
            expected_low,
        );
        ...
        let expected_high =
            7.0 + high_ph_mv / (cal.slope_high * t_measured.kelvin() / t_ideal.kelvin());
        assert_float_equality(
            cal.get_calibrated_ph_measurement(t_measured, Voltage::from_mv(high_ph_mv))
                .ph_value,
            expected_high,
        );
    }
