...
mod tests {
    ...
    fn test_append_digit() {
        ...
        assert_eq!(
            append_digit("1".into(), "".into()),
            <&str as Into<SharedString>>::into("1")
        );
        assert_eq!(
            append_digit("2".into(), "".into()),
            <&str as Into<SharedString>>::into("2")
        );
        assert_eq!(
            append_digit("3".into(), "".into()),
            <&str as Into<SharedString>>::into("3")
        );
        assert_eq!(
            append_digit(".".into(), "1".into()),
            <&str as Into<SharedString>>::into("1.")
        );
        assert_eq!(
            append_digit(".".into(), "12".into()),
            <&str as Into<SharedString>>::into("12.")
        );
        assert_eq!(
            append_digit(".".into(), "123".into()),
            <&str as Into<SharedString>>::into("123.")
        );

        // Appending a second decimal point should be ignored
        assert_eq!(
            append_digit(".".into(), "1234.5".into()),
            <&str as Into<SharedString>>::into("1234.5")
        );

        // Appending a leading zero should be allowed only once
        assert_eq!(
            append_digit("0".into(), "".into()),
            <&str as Into<SharedString>>::into("0")
        );
        assert_eq!(
            append_digit("0".into(), "0".into()),
            <&str as Into<SharedString>>::into("0")
        );

        // Appending a decimal point without a leading zero should add one
        assert_eq!(
            append_digit(".".into(), "".into()),
            <&str as Into<SharedString>>::into("0.")
        );

        // Appending a decimal point with a leading zero should be preserve it
        assert_eq!(
            append_digit(".".into(), "0".into()),
            <&str as Into<SharedString>>::into("0.")
        );

        // Appending a non-decimal digit should erase any leading zeros
        assert_eq!(
            append_digit("1".into(), "0".into()),
            <&str as Into<SharedString>>::into("1")
        );
        assert_eq!(
            append_digit("1".into(), "00".into()),
            <&str as Into<SharedString>>::into("001")
        );
    }
