...
impl EnableSensor {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_enable_sensor(move |sensor_type| {
...
impl DisableSensor {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_disable_sensor(move |sensor_type| {
...
impl SetSensorMinValue {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_set_sensor_min_value(move |sensor_type, min_value| {
            send(SetSensorMinValue { sensor_type, min_value });
        });
    }
    ...
    pub async fn handle<'a, Rtc: RealTimeClock, Sensors: SensorReadRaw, Pumps: PumpController, S: Storage<Error = E>, E: Debug>(
        ...
        update_device_config(ctx.config_buffer, ctx.current_timestamp, |device_config| {
            match self.sensor_type {
                ...
                SensorType::Temperature => device_config.temperature.min_acceptable = self.min_value,
            }
        }).await;
    }
}
...
impl SetSensorMaxValue {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_set_sensor_max_value(move |sensor_type, max_value| {
            send(SetSensorMaxValue { sensor_type, max_value });
        });
    }
    ...
    pub async fn handle<'a, Rtc: RealTimeClock, Sensors: SensorReadRaw, Pumps: PumpController, S: Storage<Error = E>, E: Debug>(
        ...
        update_device_config(ctx.config_buffer, ctx.current_timestamp, |device_config| {
            match self.sensor_type {
                ...
                SensorType::Temperature => device_config.temperature.max_acceptable = self.max_value,
            }
        }).await;
    }
}
...
impl SetThermistorBeta {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_set_thermistor_beta(move |beta_value| {
...
impl ReadSensorRaw {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_measure_sensor_raw_value(move |sensor_type| {
    ...
    pub async fn handle<'a, Rtc: RealTimeClock, Sensors: SensorReadRaw, Pumps: PumpController, S: Storage<Error = E>, E: Debug>(
        ...
        let result: Result<f32, _> = match self.sensor_type {
            SensorType::Ph => tc.sensor_controller.measure_ph_voltage().await.map(|v| v.mv()),
            SensorType::Orp => tc.sensor_controller.measure_orp_voltage().await.map(|v| v.mv()),
            SensorType::Temperature => tc.sensor_controller.measure_temperature_resistance().await.map(|r| r.ohms()),
            SensorType::Conductivity => tc.sensor_controller.measure_ec_resistance().await.map(|r| r.ohms()),
        };
        match result {
            Ok(value) => {
...
impl ReadSensorCalibrated {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_measure_sensor_calibrated_value(move |sensor_type| {
        ...
        match self.sensor_type {
            ...
            SensorType::Ph => {
                ...
                let temperature = match config.temperature.beta_value {
                    Some(beta) => tc.sensor_controller.measure_temperature(beta).await.unwrap_or_default(),
            ...
            SensorType::Conductivity => {
                ...
                match tc.sensor_controller.measure_conductivity(ec_cal, temperature).await {
                    Ok(measurement) => {
                        crate::ui_backend::state::update_manual_sensor_reading(
                            self.sensor_type,
...
impl FetchSensorChart {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<SensorUiState>().on_view_history(move |sensor_type| {
...
impl MeasureAndCalibrateEc {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<crate::ui_types::WorkflowUiState>().on_trigger_ec_calibration(move |solution_us_per_cm| {
    ...
    pub async fn handle<'a, Rtc: RealTimeClock, Sensors: SensorReadRaw, Pumps: PumpController, S: Storage<Error = E>, E: Debug>(
        ...
        let beta = match crate::storage::get_device_config().await.temperature.beta_value {
            Some(beta) => beta,
            None => {
        ...
        match tc.sensor_controller.measure_ec_resistance().await {
            Ok(resistance) => {
                ...
                log::info!("EC calibration saved: K={:.3} cm⁻¹ ({:.0} µS/cm, {:.1}°C)", calibration.cell_constant, self.solution_us_per_cm, temperature.celsius());
            }
            Err(e) => {
...
impl MeasureAndCalibrateOrp {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<crate::ui_types::WorkflowUiState>().on_trigger_orp_calibration(move |solution_mv| {
    ...
    pub async fn handle<'a, Rtc: RealTimeClock, Sensors: SensorReadRaw, Pumps: PumpController, S: Storage<Error = E>, E: Debug>(
        ...
        match tc.sensor_controller.measure_orp_voltage().await {
            Ok(measured_voltage) => {
                ...
                let calibration = OrpCalibration::new(measured_voltage, solution_voltage, ctx.current_timestamp);
                drop(tc);
                update_device_config(ctx.config_buffer, ctx.current_timestamp, |device_config| {
                    device_config.orp.calibration = Some(calibration);
                }).await;
                log::info!("ORP calibration saved: {:.0} mV measured, {:.0} mV solution", measured_voltage.mv(), self.solution_mv);
            }
            Err(e) => {
...
impl SavePhCalibration {
    pub fn register_callback(ui: &MainWindow, send: impl Fn(Self) + 'static + Clone) {
        ui.global::<crate::ui_types::WorkflowUiState>().on_save_ph_calibration(move |
            low_ph, low_mv, low_temp_c,
            mid_ph, mid_mv, mid_temp_c,
            high_ph, high_mv, high_temp_c
        | {
    ...
    pub async fn handle<'a, Rtc: RealTimeClock, Sensors: SensorReadRaw, Pumps: PumpController, S: Storage<Error = E>, E: Debug>(
        ...
        let low_point = PhMeasurementPoint::new(self.low_ph, Voltage::from_mv(self.low_mv), Temperature::from_celsius(self.low_temp_c));
        let mid_point = PhMeasurementPoint::new(self.mid_ph, Voltage::from_mv(self.mid_mv), Temperature::from_celsius(self.mid_temp_c));
        let high_point = PhMeasurementPoint::new(self.high_ph, Voltage::from_mv(self.high_mv), Temperature::from_celsius(self.high_temp_c));

        let calibration = ThreePointPhCalibration::new(high_point, mid_point, low_point, ctx.current_timestamp);
